...
class Lib {
	...
	public static var current(get_current, null):Stage;
	...
	private static var mCurrent:Stage;
	private static var mForce2DTransform:Bool;
	private static var mMainClassRoot:Stage;
	private static var mMe:Lib;
	private static var mStage:Stage;

    public static var renderer(get_renderer, null):Dynamic;
    private static var mRenderer:Dynamic;
	...
	public static function attach(name:String):Stage
	{
		return new Stage();
	}
	...
	public static function nmeGetStage():Stage {
		...
		if (mStage == null) {
            ...
            mStage = new Stage(0xFF0000,true);

            mRenderer = Detector.autoDetectRenderer(width, height);




            var tgt:DivElement = if (Lib.mMe != null) Lib.mMe.__scr; else cast Browser.document.getElementById(NME_IDENTIFIER);
            tgt.appendChild(renderer.view);
            //TODO where to call renderer?
            renderer.render(mStage.pixiDisplayObject);
		}
	...
	// Getters & Setters
	 ...
	 private static function get_renderer():Dynamic {
        return mRenderer;

     }
	
	
	private static function get_current():Stage {
		//trace("Lib get_current (Stage)");
		if (mMainClassRoot == null) {
			
			mMainClassRoot = nmeGetStage(); //new Stage();
